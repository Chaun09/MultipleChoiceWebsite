    ...
    <?php include("student-navi.php");?>
        ...
        <div id="all-quiz" class="tabcontent">
            <div class="quiz-container">
                ...
                <div class="toanroirac-quiz-col">
                    <div class="title-btn">
                     

                        <p>Toan Roi Rac</p>
                        <button onclick="location.href='show-quiz.php?cat=Toan Roi Rac'">View More</button>
                    </div>
                    <div class="quiz-box">
                        <?php
                        include("config.php");
                        $sql = "SELECT *, (SELECT COUNT(quques_id) FROM quiz_question qq WHERE (qq.quiz_id = q.quiz_id)) AS totalquestion 
                        , (SELECT COUNT(stud_id) FROM history h WHERE (h.quques_id = qq.quques_id)) AS totalplays FROM quiz q INNER JOIN quiz_question qq WHERE (qq.quiz_id = q.quiz_id) AND (q.quiz_category = 'Toan Roi Rac') GROUP BY q.quiz_id ORDER BY RAND() LIMIT 4";
                        $result = mysqli_query($conn, $sql);
                        while ($row = mysqli_fetch_array($result)){
                        ?>
                        <a class="quiz-link" href="student-quizquestion.php?quizid=<?php echo $row['quiz_id'];?>">
